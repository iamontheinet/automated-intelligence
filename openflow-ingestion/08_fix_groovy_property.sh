#!/bin/bash
#
# Fix ExecuteGroovyScript to use correct property name
#
...
PROCESSOR_ID="13bea406-019b-1000-0000-00006c234a07"
...
echo "Fixing ExecuteGroovyScript processor properties..."
...
curl -s -X PUT -H "Authorization: Bearer $PAT" \
    ...
    "${API_BASE}/processors/${PROCESSOR_ID}" \
    -d '{
        "revision": {"version": 1},
        "component": {
            "id": "'"$PROCESSOR_ID"'",
            "config": {
                "properties": {
                    "groovyx-script-body": '"$(echo "$GROOVY_SCRIPT" | jq -Rs .)"'
                }
            }
        }
    }' > /dev/null

echo "✅ ExecuteGroovyScript processor fixed"
echo ""
echo "The processor now uses the correct property: 'groovyx-script-body'"
echo "Ready to start in Openflow UI!"
